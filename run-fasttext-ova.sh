...
for type in "${types[@]}"; do
  echo $type
  # ./fastText-0.9.2/fasttext supervised -input $FOLDER/${type}_unfiltered.train.txt -output $FOLDER/${type}_unfiltered_model -epoch 100 -lr 0.5 -loss one-vs-all
  ...
  for b in "${bys[@]}"; do
    echo $b
    # ./fastText-0.9.2/fasttext supervised -input $FOLDER/${type}_${b}_filtered.train.txt -output $FOLDER/${type}_${b}_filtered_model -epoch 100 -lr 0.5 -loss one-vs-all
